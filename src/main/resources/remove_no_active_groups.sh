...
for i in $dir
do
    /root/kafka_2.11-1.1.1/bin/kafka-consumer-groups.sh --bootstrap-server 192.168.223.100:9092,192.168.223.101:9092,192.168.223.102:9092 --describe --members --group $i | grep 'has no active members' | awk -F" " '{ print $3; }'
done